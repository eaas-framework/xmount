...
//SEM// Semaphores to control concurrent read & write access
//SEMstatic sem_t sem_img_read;
//SEMstatic sem_t sem_vdiheader_read;
//SEMstatic sem_t sem_info_read;
//SEMstatic sem_t sem_cachefile_rw;
...
//SEM  sem_wait(&sem_cachefile_rw);
  if(pCacheFileHeader->VdiFileHeaderCached==1) {
//SEM    sem_post(&sem_cachefile_rw);
    return TRUE;
  } else {
//SEM    sem_post(&sem_cachefile_rw);
    return FALSE;
...
//SEM          sem_wait(&sem_vdiheader_read);
          memcpy(buf,((char*)pVdiFileHeader)+FileOff,CurToRead);
//SEM          sem_post(&sem_vdiheader_read);
  ...
  while(ToRead!=0) {
    ...
    if(XMountConfData.Writable==TRUE && IsBlockCached(CurBlock)==TRUE) {
...
//SEM  // Unlock cache file and return
//SEM  sem_post(&sem_cachefile_rw);
  return size;
}
...
//SEM  // Init semaphores
//SEM  sem_init(&sem_img_read,0,1);
//SEM  sem_init(&sem_vdiheader_read,0,1);
//SEM  sem_init(&sem_info_read,0,1);
//SEM  sem_init(&sem_cachefile_rw,0,1);
...
//SEM  // TODO: Perhaps wait for unposted sem's
//SEM  sem_destroy(&sem_cachefile_rw);
//SEM  sem_destroy(&sem_info_read);
//SEM  sem_destroy(&sem_vdiheader_read);
//SEM  sem_destroy(&sem_img_read);
...
/*
  ...
  20090327: v0.2.1 released
            ...
            * Added IsBlockCached function to check whether a block was already
              cached
  20090410: * Reverted most of the fixes from v0.2.1 and 0.2.2 as those did not
              solve the problem.
