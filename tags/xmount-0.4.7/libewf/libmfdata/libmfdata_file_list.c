		...
		*file_list         = NULL;
		...
		if( ( internal_file_list->flags & LIBMFDATA_FLAG_IO_HANDLE_MANAGED ) != 0 )
		{
			if( internal_file_list->io_handle != NULL )
			{
				...
				else if( internal_file_list->free_io_handle(
					  internal_file_list->io_handle,
					  error ) != 1 )
				{
					liberror_error_set(
     ...
     liberror_error_t **error )
{
	...
	if( libmfdata_cache_get_number_of_entries(
	     cache,
	...
	if( ( read_flags & LIBMFDATA_READ_FLAG_IGNORE_CACHE ) == 0 )
	{
		...
		if( libmfdata_cache_get_value_by_index(
		     cache,
		     cache_entry_index,
		     &cache_value,
		     error ) != 1 )
		...
		if( cache_value != NULL )
		{
			...
			if( libmfdata_cache_value_get_identifier(
			     cache_value,
	...
	if( libmfdata_cache_value_get_value(
	     cache_value,
	     file_value,
	     error ) != 1 )
	{
     ...
     liberror_error_t **error )
{
	...
	if( libmfdata_cache_set_value_by_index(
	     cache,
	     cache_entry_index,
	     file_io_pool_entry,
	     0,
