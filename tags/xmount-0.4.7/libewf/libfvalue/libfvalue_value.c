     ...
     liberror_error_t **error )
{
	...
	if( *value != NULL )
	{
		if( libfvalue_value_free_as_value(
		     (intptr_t *) *value,
		     error ) != 1 )
		{
			liberror_error_set(
 ...
 * Returns 1 if successful or -1 on error
 */
int libfvalue_value_free_as_value(
     intptr_t *value,
     liberror_error_t **error )
{
	libfvalue_internal_value_t *internal_value = NULL;
	static char *function                      = "libfvalue_value_free_as_value";
	int result                                 = 1;
     ...
     liberror_error_t **error )
{
	...
	if( libfvalue_value_initialize(
	     ...
	     LIBFVALUE_VALUE_FLAG_IDENTIFIER_MANAGED | LIBFVALUE_VALUE_FLAG_DATA_MANAGED | LIBFVALUE_VALUE_FLAG_METADATA_MANAGED,
	     error ) != 1 )
	{
		liberror_error_set(
		 error,
     ...
     liberror_error_t **error )
{
	...
	if( ( internal_value->flags & LIBFVALUE_VALUE_FLAG_DATA_MANAGED ) == 0 )
	{
		liberror_error_set(
		 ...
		 "%s: invalid value - data managed flag is not set.",
		 function );
	...
	internal_value->data_size *= number_of_value_entries;
     ...
     liberror_error_t **error )
{
	...
	else
	{
		...
		else if( ( flags & LIBFVALUE_VALUE_DATA_FLAG_MANAGED ) != 0 )
		{
			internal_value->data = (uint8_t *) data;
		}
		else
	...
	else
	{
		...
		else
		{
			internal_value->metadata = (uint8_t *) memory_allocate(
								sizeof( uint8_t ) * internal_value->metadata_size );
			...
			if( memory_copy(
			     internal_value->metadata,
			     metadata,
			     internal_value->metadata_size ) == NULL )
			{
				liberror_error_set(
				 error,
				 LIBERROR_ERROR_DOMAIN_MEMORY,
...
/* TODO multi value support */
	...
	if( internal_value->data != NULL )
	{
		if( ( internal_value->flags & LIBFVALUE_VALUE_FLAG_DATA_MANAGED ) == 0 )
		{
			if( internal_value->data_size < utf8_string_size )
			{
				liberror_error_set(
				 error,
				 LIBERROR_ERROR_DOMAIN_RUNTIME,
				 LIBERROR_RUNTIME_ERROR_VALUE_OUT_OF_BOUNDS,
				 "%s: value data size too small.",
				 function );
...
/* TODO multi value support */
	...
	if( internal_value->data != NULL )
	{
		if( ( internal_value->flags & LIBFVALUE_VALUE_FLAG_DATA_MANAGED ) == 0 )
		{
			if( internal_value->data_size < utf16_string_size )
			{
				liberror_error_set(
				 error,
				 LIBERROR_ERROR_DOMAIN_RUNTIME,
...
/* TODO multi value support */
	...
	if( internal_value->data != NULL )
	{
		if( ( internal_value->flags & LIBFVALUE_VALUE_FLAG_DATA_MANAGED ) == 0 )
		{
			if( internal_value->data_size < utf32_string_size )
			{
				liberror_error_set(
				 error,
				 LIBERROR_ERROR_DOMAIN_RUNTIME,
