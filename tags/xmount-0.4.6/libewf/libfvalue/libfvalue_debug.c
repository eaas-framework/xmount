     ...
     liberror_error_t **error )
{
...
#if defined( LIBFVALUE_DEBUG_HAVE_LIBFDATETIME )
	libcstring_system_character_t date_time_string[ 24 ];

	libfdatetime_filetime_t *filetime           = NULL;
#endif

#if defined ( LIBFVALUE_DEBUG_HAVE_LIBFGUID )
	...
	if( number_of_value_entries == 0 )
	{
		switch( value_type )
		{
			...
			case LIBFVALUE_VALUE_TYPE_UNIXTIME:
				break;
			...
			case LIBFVALUE_VALUE_TYPE_FILETIME:
				...
				if( result != 0 )
				{
					...
					{
						libnotify_printf(
						 "%s: Filetime\t: %" PRIs_LIBCSTRING_SYSTEM "\n",
						 function,
						 date_time_string );
					}
				}
				...
				break;
#endif /* defined( LIBFVALUE_DEBUG_HAVE_LIBFDATETIME ) */
			...
			case LIBFVALUE_VALUE_TYPE_GUID:
				...
				break;
#endif /* if defined ( LIBFVALUE_DEBUG_HAVE_LIBFGUID ) */
