...
static void PrintUsage(char *pProgramName) {
  ...
  printf("          /etc/fuse.conf and use the -o allow_root command line parameter.\n");
  printf("  mopts:\n");
//  printf("    --debug : Enable xmount's debug mode.\n");
  ...
  printf("    --out <otype> : Output image format. <otype> can be \"dd\", \"vdi\", \"vmdk\".\n");
  ...
  printf("    segments! (If your shell supports it, you can use .E?? as file\n");
  printf("    extension to specify all EWF files)\n");
#else
  printf("\n");
#endif
  printf("  mntp:\n");
